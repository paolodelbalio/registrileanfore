...
function elaboraDatiTabella(chiave, righeGrezze) {
    ...
    // Mensile) nelle colonne oltre "Firma". La escludiamo dalla tabella: viene mostrata a parte,
    // come blocco statico in fondo alla sezione (vedi index.html + mostraLegendaPulizie()).
    if (chiave === 'pulizie') {
        const colonnePulizieValide = ["Data", "Ora", "Area Pulita", "Intervento", "Prodotto Usato", "Incaricato", "Firma"];
        intestazioni = intestazioniOriginali.filter(h => colonnePulizieValide.includes(h));
    }
    ...
    for (let i = 1; i < righeGrezze.length; i++) {
        ...
        intestazioniOriginali.forEach((intestazione, indice) => {
            if (chiave === 'pulizie' && !intestazioni.includes(intestazione)) return;
...
function creaTabellaStandard(chiaveTabella, intestazioni, dati) {
    ...
    html += "</tbody>";
    tabella.innerHTML = html;
    if (chiaveTabella === 'pulizie') creaLegendaPulizie();
}
